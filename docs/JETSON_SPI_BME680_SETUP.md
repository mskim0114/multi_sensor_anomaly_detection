...
2. **CSB 가 한 번이라도 LOW 였다면 진짜 power-on reset 이 필요하다.** 그 전까지 I2C 는
   ...
   충분하지 않다.
3. **I2C 풀업.** Jetson Orin Nano Developer Kit carrier board 는 물리
   **Pin 3 (SDA) / Pin 5 (SCL) 에 3.3 V 2.2 kΩ pull-up 을 제공한다.** 따라서 이 버스에서는
   모듈을 단독으로 붙여도 통신이 성립한다.

   > **정정 (2026-09-03).** 이전 판에는 "Jetson 40핀 헤더는 풀업을 제공하지 않는다" 고
   > 적혀 있었다. 사실이 아니다. 이 정정은 사실 관계만 바로잡는 것이고 추가 electrical
   > debugging 은 수행하지 않았다.
...
두고, 그 장치가 응답하는데 BME680 만 무응답일 때에만 모듈 문제로 판정한다. (Pin 3/5 의
풀업은 carrier board 가 제공하므로, 다른 모듈을 빼는 것 자체가 풀업을 없애지는 않는다 — §9-1.)
