...
Predicting equipment degradation in manufacturing facilities is critical for ensuring worker safety and preventing costly unplanned downtime. While recent approaches employ complex Transformer architectures for multimodal sensor fusion, their effectiveness is often limited by insufficient training data in industrial settings. In this paper, we propose a lightweight yet effective approach that combines multi-scale temporal difference features with supervised contrastive learning for multimodal sensor-based degradation prediction. Our method extracts rate-of-change signals at multiple time lags (1, 5, and 10 steps) from eight sensor channels (temperature, particulate matter, and motor current) and fuses them with thermal imagery through a Long Short-Term Memory (LSTM) network augmented with channel attention over its latent features (the fusion is additive and the thermal branch is aggregated by a temporal mean, so the model does not represent cross-modal timing). A joint loss function combining cross-entropy and supervised contrastive loss explicitly separates the latent representations of closely overlapping degradation states. We evaluate our approach on a large-scale manufacturing dataset comprising 111,870 multimodal samples from 36 industrial transport devices (OHT and AGV). Our proposed model achieves a macro F1-score of 0.9557 +/- 0.0006 (three runs) with only 2.85M parameters, outperforming a Cross-Attention Temporal Fusion Transformer (10.8M parameters, F1 = 0.9252) by 3.05%. A comprehensive ablation study across eight model variants (plus three component-level ablations) shows that each of the three proposed components—multi-scale differences, channel attention, and contrastive loss—is individually ineffective or slightly harmful, while the combined configuration is +1.20% F1 above the single-lag variant; because the combined configuration was trained for 30 epochs against the 20 epochs used for the single-component runs, and each entry is one run, we report this as an observation and not as a measured interaction. Our model classifies every Severe-labelled validation window correctly (66 of 66 windows) and, in its seed-42 run, reduces Normal-Mild boundary misclassifications by 47% relative to the single-lag variant; the other two runs give reductions of 36% and 27%, so we report the range rather than the best value. The model is exported to ONNX format (10.9 MB) and measured on an NVIDIA Jetson Orin Nano at 16.27 ms per sample through the ONNX Runtime CUDA execution provider, which is well inside the 1 Hz monitoring cadence this system targets.
...
Two caveats apply to this table before the effects are read as an interaction. First, the V2, V2a, V2b and V2c runs used a 20-epoch budget while V2+ used 30 epochs, and V2+ reached its best validation F1 at epoch 27, after the learning-rate scheduler had already reduced the rate at epoch 25; the V2 run's learning-rate trace was not recorded, so we cannot confirm that the two arms used the same schedule. Second, every entry is a single run with an uncontrolled initialisation. **[PENDING RE-EXPERIMENT: EXP-20260907-003]** A 2^3 design at a matched budget, a matched scheduler and multiple seeds is required before the combined effect is attributed to an interaction between the three components. With that caveat, in this table each component individually provides marginal or even negative improvement. Multi-scale diff alone (+0.02%) suggests that the LSTM can already extract some multi-scale information from single-lag features. SE attention alone (-1.11%) overfits the channel gating mechanism without sufficiently rich input features. SupCon loss alone (-0.08%) shows that contrastive learning on the original feature space provides limited benefit.
...
| **H2** | The contrastive term reduces Normal-Mild misclassifications vs. the single-lag baseline (threshold of 40 % was formalised after the runs, not preregistered) | Normal-Mild errors: V2 = 45 -> V2+ = 24 for the seed-42 run, a 46.7 % reduction, while macro F1 improves from 0.9430 to 0.9557 +/- 0.0006. The F1 figure aggregates three runs but the error count is from one run only; the other two V2+ runs give 29 and 33 Normal-Mild errors. The hypothesis names the contrastive loss, but the V2 -> V2+ transition also changes the lag set and adds SE attention, and the contrastive term applied alone (V2c) leaves Normal-Mild errors slightly worse than V2 (46 vs. 45; Table 8). The implementation weight is 0.3, not 0.1 | Table 8, Table 10 (confusion matrices) | **[PENDING RE-EXPERIMENT: EXP-20260907-003]** Direction reproduced; the >= 40 % threshold cannot be judged from a single run and the attribution to the contrastive term alone is not supported |
