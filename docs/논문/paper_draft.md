...
Predicting equipment degradation in manufacturing facilities is critical for ensuring worker safety and preventing costly unplanned downtime. While recent approaches employ complex Transformer architectures for multimodal sensor fusion, their effectiveness is often limited by insufficient training data in industrial settings. In this paper, we propose a lightweight yet effective approach that combines multi-scale temporal difference features with supervised contrastive learning for multimodal sensor-based degradation prediction. Our method extracts rate-of-change signals at multiple time lags (1, 5, and 10 steps) from eight sensor channels (temperature, particulate matter, and motor current) and fuses them with thermal imagery through a Long Short-Term Memory (LSTM) network augmented with channel attention over its latent features (the fusion is additive and the thermal branch is aggregated by a temporal mean, so the model does not represent cross-modal timing). A joint loss function combining cross-entropy and supervised contrastive loss explicitly separates the latent representations of closely overlapping degradation states. We evaluate our approach on a large-scale manufacturing dataset comprising 111,870 multimodal samples from 36 industrial transport devices (OHT and AGV). Our proposed model achieves a macro F1-score of 0.9557 +/- 0.0006 (three runs) with only 2.85M parameters, outperforming a Cross-Attention Temporal Fusion Transformer (10.8M parameters, F1 = 0.9252) by 3.05%. A comprehensive ablation study across eight model variants, followed by a 2^3 factorial ablation of the three proposed components (multi-scale differences, channel attention, contrastive loss; three seeds per cell, matched 30-epoch budget), shows that multi-scale differences account for about 60% of the +1.69 percentage-point gain over the single-lag variant (+1.16 pp main effect), the contrastive term adds +0.42 pp, channel attention has no detectable main effect (+0.01 ± 0.11 pp), and interactions among the three account for roughly a quarter of the total gain. Our model classifies every Severe-labelled validation window correctly (66 of 66 windows) and, in its seed-42 run, reduces Normal-Mild boundary misclassifications by 47% relative to the single-lag variant; the other two runs give reductions of 36% and 27%, so we report the range rather than the best value. The model is exported to ONNX format (10.9 MB) and measured on an NVIDIA Jetson Orin Nano at 16.27 ms per sample through the ONNX Runtime CUDA execution provider, which is well inside the 1 Hz monitoring cadence this system targets.
...
Recent work in predictive maintenance has explored deep learning approaches for multimodal sensor fusion. Transformer-based architectures with cross-attention fusion have shown promise in modeling complex inter-modal relationships [5]. However, these architectures typically require large training datasets to avoid overfitting, which is often impractical in industrial settings where labeled anomaly data is scarce [7]. Furthermore, their computational overhead may hinder deployment on resource-constrained edge devices such as the NVIDIA Jetson platform, which is commonly used for on-site inference in manufacturing environments [8].
...
- We propose a **joint cross-entropy and supervised contrastive loss** that explicitly separates the latent representations of closely overlapping degradation states. In the factorial ablation it adds +0.42 pp F1 on top of the multi-scale features; the full model reduces Normal-Mild misclassifications by 27–47% relative to the single-lag variant across three runs, most of which is attributable to the multi-scale features rather than to the contrastive term alone.
...
- **RQ2 (Explicit class separation in latent space).** Can a supervised contrastive objective, applied jointly with cross-entropy, resolve the Normal-Mild boundary that is difficult to separate under standard classification loss alone?
  - **H2.** Adding a supervised contrastive term (implementation weight 0.3; an earlier draft of this hypothesis stated 0.1, which no run used) on top of the multi-scale-diff LSTM backbone reduces Normal-Mild misclassifications relative to the single-lag temporal-difference model, while preserving or improving overall macro F1. The 40 % threshold used in Section 6.9 was formalised after the runs were complete and is not a preregistered criterion.
...
Each hypothesis is designed to be **falsifiable** against the experiments defined in Section 5. We revisit H1-H3 explicitly in Section 6.9, mapping each hypothesis to the specific tables and metrics that bear on it. The hypothesis numbering is retained from the original draft; where the evidence turned out not to support a hypothesis as stated, Section 6.9 records that rather than restating the hypothesis.
...
The provider labels the four current channels as an overcurrent sensor group (unit A, sensor code S18) and the distributed package does not document which line or motor each channel measures. The per-channel mapping used below is taken from the provider's utilization guideline, which is not part of the distributed data package, and we could not verify it against any shipped file: CT1 = input line, CT2 = output line, CT3 = motor 1, CT4 = motor 2. This physical role difference explains the pronounced scale disparity across channels (CT2 mean 42.73 A versus CT1 mean 6.23 A) and the different degradation-time behaviors seen in Section 3.4.

The thermal infrared images are distributed as `.bin` files whose contents are a NumPy `.npy` byte stream holding a 120 x 160 array of °C values. The stored resolution should not be confused with the native sensor resolution: the acquisition camera (Section 3.2b) has a 32 x 32 native pixel grid, so the 120 x 160 representation is the result of the dataset provider's ~15 x upsampling. We use the released 120 x 160 arrays directly. Values range approximately from 31 °C (background) to 146 °C (highest observed hot spot), remaining consistent with the guideline range and requiring no clipping.
...
| Current transformer | KEMET **CT-06** (part identification from the provider's utilization guideline, not from the distributed package; shipped files carry only the sensor code S18) | Passive CT; provider maps to A. Reported CT2 peaks at 273.91 A exceed the nominal spec (0-200 A) — treated as valid data |
...
Given a temporal window of T consecutive multimodal samples, each consisting of an 8-dimensional sensor vector s_t in R^8 and a thermal image I_t in R^{120x160}, the task is to predict the degradation state y in {0, 1, 2, 3} for the window as defined in Section 3.3. We use a window size of T = 30 corresponding to 30 seconds at the released 1 Hz sampling rate. Note that per the provider's utilization guideline (a document not included in the distributed data package; the shipped label files record only `duration_time = 1`) this 1 Hz series is itself a 10-to-1 temporal aggregation of an internal 100 ms (10 Hz) acquisition; the aggregation function (mean / max / last) is not disclosed, so any temporal analysis in this paper is bounded from below by the 1 s aggregation window. The training objective is to maximize macro-averaged F1-score while ensuring 100 % recall for the Danger state (which corresponds, per Section 3.3, to the final 30 seconds before carbonization).
...
**Class-balanced sampling.** We employ a WeightedRandomSampler that assigns sampling weights inversely proportional to class frequency. This is combined with class-weighted cross-entropy loss (weights: Normal = 0.28, Mild = 0.60, Moderate = 0.61, Severe = 2.51). The two mechanisms compound rather than cancel: the sampler already equalises the four classes within each epoch, and the loss weights then scale each class again by the inverse of its frequency, so the effective share of the loss gradient is approximately 7 % Normal, 15 % Mild, 15 % Moderate and 63 % Severe against a data frequency of 49.5 / 21.3 / 21.4 / 7.7 %. The training objective is therefore biased toward the minority classes rather than balanced, which is consistent with the Severe recall reported in Section 6.7. A consequence is that the softmax outputs are calibrated to this reweighted prior and not to the field prior, so they should not be read as deployment probabilities.
...
|:------|:------:|:-------:|:----------:|-------:|---:|
| AI Hub provider-reported figure [32] (architecture and protocol not disclosed) | 0.9109 | 91.92% | n/a | n/a | n/a |
...
All models except V2+ are single runs. V2+ reports mean +/- std over three independent initialisations (nominally seeds 42, 123 and 456; the seed was applied after the model was constructed in the original scripts, so it labels the runs but does not make them bit-reproducible - see Limitations). See Table 12 for details. The AI Hub provider-reported figure is taken from the dataset provider's utilization guideline [32]; fields not disclosed there are marked "n/a". We could not verify this figure against the distributed package: the model actually shipped by the provider is a multimodal LSTM + CNN (3-layer LSTM over the 8 sensor channels, a per-frame CNN over the thermal image, concatenation and a single linear head; 2,833,412 parameters, with no attention tensors in the released checkpoint), and it is architecturally the same model as our V1. The "MMTransformer" name that appears in the provider's readme and manual refers to an unrelated trajectory-prediction repository and does not describe the shipped code or weights. The evaluation protocol behind 0.9109 is also undisclosed: the provider code that is included uses stride-1 windows split at random, takes the first timestep as the window label, applies no normalization, and orders the particulate channels differently from the released CSV header. The figure is therefore reported here for completeness and is not directly comparable to our results.

**[Figure 3: Model performance comparison with parameter count overlay.]**

The proposed V2+ model achieves the highest macro F1-score of 0.9557 +/- 0.0006 (mean over three runs), outperforming every comparison method that we trained and evaluated ourselves under one protocol: the provider's shipped architecture retrained in our pipeline (V1, F1 = 0.9235, 2,833,412 parameters), recent general-purpose time series architectures (TimesNet, PatchTST), and the substantially larger internal CATFT variant (V5, 10.79M parameters). Against V1 — the provider's own architecture, evaluated on the same equipment-disjoint split, the same labels and the same normalization — V2+ improves macro F1 by 3.22 percentage points (0.9235 → 0.9557) at essentially the same parameter count. The provider-reported 0.9109 in Table 6 is not used for a difference claim, because neither its architecture nor its evaluation protocol is disclosed in the distributed package. Compared to PatchTST, V2+ improves F1 by 2.46 % while using approximately twice the parameters. **[PENDING RE-EXPERIMENT: EXP-20260907-001 / -004]** The comparison against TimesNet is withheld: our TimesNet implementation reconstructs the dominant period from the wrong rFFT index, uses a single period instead of the original multi-period aggregation, and omits the inception blocks, so it is a reduced variant rather than TimesNet.

One pattern emerges from Table 6. Our two cross-attention variants — V4 CATFT-CrossAttn (0.9112) and the fully-featured V5 (0.9252) — both land in the 0.91-0.93 F1 range under our protocol. Two variants of one internal architecture are not sufficient evidence for an empirical ceiling on cross-attention fusion, and we do not claim one; the provider-reported figure cannot be used as a third, independent data point because the shipped provider model contains no attention at all. Second, both external general-purpose baselines (TimesNet, PatchTST) underperform even our V2 variant (LSTM + single-lag temporal difference), reinforcing that domain-informed temporal features are more effective than generic time-series architectures for this task.
...
To isolate the contribution of each component added in V2+, we ran a 2^3 factorial ablation: every combination of multi-scale differences (lags [1,5,10] vs. [1]), SE channel attention (on/off) and the supervised contrastive term (weight 0.3 vs. cross-entropy only), each with three seeds (42, 123, 456). All 24 runs used the same 30-epoch budget, the same optimiser and plateau-scheduler rule, the same best-validation-F1 checkpoint rule, fp32 training and evaluation, seed-controlled initialisation, and dropout 0.1 before the classifier in every configuration. An earlier single-seed table (V2, V2a, V2b and V2c trained for 20 epochs against V2+ for 30) is superseded by this design and is no longer reported.

**Table 8.** 2^3 factorial ablation of the V2+ components (macro F1 mean ± std over three seeds; Normal-Mild (NM) errors are three-seed means; ΔF1 in percentage points vs. the all-off V2 cell).

| Configuration | Multi-scale | SE | SupCon | F1 (mean ± std) | ΔF1 vs V2 (pp) | NM errors |
|:-------------|:-----------:|:--:|:------:|:---------------:|:--------------:|:---------:|
| V2 (all off) | – | – | – | 0.9394 ± 0.0013 | — | 50.0 |
| + Multi-scale diff | ✓ | – | – | 0.9498 ± 0.0018 | +1.04 | 35.3 |
| + SE attention | – | ✓ | – | 0.9402 ± 0.0009 | +0.08 | 50.3 |
| + SupCon loss | – | – | ✓ | 0.9412 ± 0.0035 | +0.18 | 45.3 |
| Multi-scale + SE | ✓ | ✓ | – | 0.9477 ± 0.0033 | +0.83 | 41.3 |
| Multi-scale + SupCon | ✓ | – | ✓ | 0.9547 ± 0.0008 | +1.53 | 30.0 |
| SE + SupCon | – | ✓ | ✓ | 0.9415 ± 0.0013 | +0.21 | 45.0 |
| **V2+ (all on)** | ✓ | ✓ | ✓ | **0.9563 ± 0.0013** | **+1.69** | **30.7** |

The all-on cell is a re-run under the factorial protocol; the headline V2+ figure used elsewhere in this paper (0.9557 ± 0.0006, Section 6.1) comes from the original three runs and is not replaced by it. Main effects (average change in F1 when a factor is switched on, ± the standard deviation of the same contrast across seeds): multi-scale +1.16 ± 0.08 pp, contrastive +0.42 ± 0.06 pp, SE +0.01 ± 0.11 pp. Two-way interactions: multi-scale × SupCon +0.26 ± 0.20 pp, multi-scale × SE −0.04 ± 0.22 pp, SE × SupCon +0.08 ± 0.15 pp; the three-way term is +0.11 ± 0.05 pp. The three single-factor gains sum to +1.30 pp against +1.69 pp for the full configuration, so interactions account for about 23% of the total gain.

Two limitations remain. First, three seeds per cell on a validation split of four machines give only a coarse uncertainty; the standard deviations above are across seeds only, and a same-seed re-run of the full model moved F1 by −0.08 pp, so effects of about ±0.1 pp (the SE main effect and the three-way term) are not separated from run-to-run noise. Second, the plateau scheduler reduced the learning rate at different epochs in different runs, so the runs share a scheduler rule rather than an identical schedule. Compared with the superseded single-seed table, the apparent −1.11% effect of SE alone disappears (+0.08 ± 0.09 pp here), which we attribute to that earlier run's shorter budget and uncontrolled initialisation.

**[Figure 4: 2^3 factorial ablation of V2+. (a) F1 per configuration (mean ± std over three seeds). (b) Normal-Mild boundary errors. Multi-scale differences dominate; the contrastive term adds a smaller, consistent gain; SE attention has no detectable main effect.]**

Normal-Mild errors follow the same pattern (three-seed means): multi-scale differences alone reduce them from 50.0 to 35.3 (−29%), the contrastive term on top of multi-scale features reduces them further to 30.0 (−15%), and the full configuration reaches 30.7 (−39% vs. V2). The contrastive term alone reduces them by only 9% (50.0 → 45.3). We therefore do not attribute the boundary-error reduction to the contrastive loss alone (Section 7.3).
...
Across all model variants except the baseline V1, every Severe-labelled validation window is classified correctly (66 of 66 windows, 100 % window-level recall). These are 66 windows, not 66 independent degradation events, and window-level recall does not imply any particular warning lead time: the window label is the plurality of the 30 per-timestep labels, so a window is labelled Severe only once Severe already occupies the plurality of it. This is attributed to the unambiguous sensor signatures at State 3: temperature surges to 50.71 °C mean, and current sensors exhibit 3.5-14x increases over baseline values. The combination of balanced sampling and class-weighted loss puts a large majority of the loss gradient on the minority Severe class (Section 4.4), which is the most likely reason this class saturates first.
...
The thermal modality provides a modest but consistent improvement of +0.37 % F1 and reduces Normal-Mild errors by 8 cases. The magnitude of this contribution is best interpreted with the source-hardware constraint disclosed in Section 3.2: the acquisition camera (Terabee Evo Thermal 33) has a 32 x 32 native pixel grid, and the 120 x 160 arrays we consume are the result of the provider's ~15 x upsampling. The spatial information available to any thermal branch is therefore bounded by the ~1,024 native pixels, not by the 19,200 stored pixels. The +0.37 % F1 gain may reflect the ceiling of this ~1 K-pixel information budget, but the pixel budget is not the only candidate bottleneck. Our fusion averages the per-frame thermal features over the 30 timesteps and then applies a linear projection with no subsequent nonlinearity, so the thermal contribution reduces to a linear function of the time-averaged feature map: permuting the thermal frames in time leaves the logits unchanged to floating-point precision, and the two modalities enter the classifier additively. Under that structure, raising the native pixel count need not raise the thermal contribution. Camera placement, the label's own dependence on the sensor channels, the single overheating process in this dataset, and redundancy with the NTC channel are further uncontrolled explanations. A deployment on the higher-resolution FLIR Lepton 3.5 (160 x 120 native, ~19,200 pixels) — with which we plan to fine-tune in the field study described in Section 7.5 — is expected to give the thermal branch materially more headroom. The sensor-only model is nonetheless noteworthy for achieving F1 = 0.9513 with only 0.37 M parameters, which may be preferable for extremely resource-constrained deployments where the thermal camera is unavailable.
...
| Inference: Jetson Orin Nano, ONNX Runtime CUDA EP (measured) | 16.27 ms |
| Inference: Jetson Orin Nano, ONNX Runtime CPU EP (measured) | 46.45 ms |
| Inference: Jetson Orin Nano, standalone TensorRT FP32 (GPU compute only) | 4.88 ms |
| Inference: Jetson Orin Nano, standalone TensorRT FP16 (GPU compute only) | 3.33 ms |
| Inference: Jetson Orin Nano, ONNX Runtime TensorRT EP | aborts on first inference |

The first three rows are desktop measurements (RTX 6000) and all fall below 10 ms. The Jetson rows are measured on the target device and do not: 16.27 ms per sample through the ONNX Runtime CUDA execution provider and 46.45 ms through the CPU provider. The two sub-5 ms Jetson figures are standalone TensorRT GPU-compute times reported by `trtexec` and exclude host-to-device transfer, device-to-host transfer and preprocessing, so they are not end-to-end latencies; the path that would deliver them inside our runtime, the ONNX Runtime TensorRT execution provider, aborts on the first inference on this device (reproduced twice). No working end-to-end path on the Jetson is currently below 10 ms. For the 1 Hz monitoring cadence this system targets, 16.27 ms is nonetheless two orders of magnitude inside budget; the claim we withdraw is the sub-10 ms one, not the feasibility of real-time operation. End-to-end latency from sensor acquisition to alarm has not yet been measured.
...
|:------:|:-----------|:-------|:---------|:-------:|
| **H1** | Multi-scale temporal differences improve F1 over single-lag and absolute-value baselines | Factorial ablation (Table 8, three seeds, matched budget): multi-scale differences have the largest main effect, +1.16 ± 0.08 pp F1, and alone account for +1.04 pp of the +1.69 pp V2 → V2+ gain; V2 vs. the V1 absolute-only baseline gives +1.95 pp (Table 7); lag sensitivity confirms [1,5,10] over [1], [1,3,7] and [1,10,20] | Table 7 (V1 vs V2), Table 8 (factorial), Table 13 (lag sensitivity) | **Supported** |
| **H2** | The contrastive term reduces Normal-Mild misclassifications vs. the single-lag baseline (threshold of 40 % was formalised after the runs, not preregistered) | Normal-Mild errors, three-seed means from the factorial ablation: V2 50.0 → contrastive term alone 45.3 (−9 %); multi-scale alone 35.3 (−29 %); multi-scale + contrastive 30.0; full V2+ 30.7 (−39 %). The original seed-42 run's 46.7 % reduction (45 → 24) is a single-run best case; the other two original runs gave 29 and 33 errors. The implementation weight is 0.3, not 0.1 | Table 8 (factorial), Table 10 (confusion matrices) | **Not supported as stated** — the full model reproduces a large reduction (−39 %, three seeds) but the isolated effect of the contrastive term is −9 %, so neither the ≥ 40 % threshold nor the attribution to the contrastive term holds |
| **H3** | V2+ (2.85 M) outperforms TimesNet, PatchTST, CATFT, and the AI Hub reference on macro F1 while remaining < 10 ms per inference | Accuracy part: V2+ F1 = 0.9557 +/- 0.0006 vs. PatchTST 0.9311, CATFT V5 0.9252 (10.79 M), V1 provider architecture 0.9235. TimesNet comparison withheld (reduced implementation) and the provider-reported 0.9109 is not protocol-comparable. Latency part: the sub-10 ms figures (ONNX GPU 2.61 ms, ONNX CPU 7.27 ms) are desktop RTX 6000 measurements; on the actual Jetson Orin Nano the measured per-sample latency is 16.27 ms (ONNX Runtime CUDA EP) and 46.45 ms (CPU EP), and the TensorRT execution provider aborts on first inference | Table 6, Table 14 (deployment) | **Accuracy part supported on our protocol; latency part NOT supported on Jetson** |

For H1, the factorial ablation isolates the temporal-difference component directly: switching to multi-scale lags raises F1 by +1.16 ± 0.08 pp averaged over all other settings and by +1.04 pp on the plain V2 baseline, so H1 is supported in its single-factor form and not only in combination with the other components.

For H2, the full model's 39 % reduction in Normal-Mild errors (three-seed means) approaches the 40 % threshold, but the factorial design shows that the contrastive term alone accounts for a 9 % reduction and multi-scale differences for 29 %; the threshold was formalised after the runs, and the attribution of the reduction to the contrastive term is not supported.
...
Our results demonstrate a counterintuitive finding: a 2.85M-parameter LSTM with domain-informed temporal features outperforms a 10.8M-parameter Transformer with cross-attention by 3.05 % F1 (0.9557 +/- 0.0006 vs. 0.9252). The comparison holds within our own protocol, where V2+ (0.9557) exceeds both cross-attention variants, V4 CATFT-CrossAttn (0.9112) and V5 CATFT (0.9252), and also the provider's shipped architecture retrained in our pipeline (V1, 0.9235). We do not extend this to a systematic ceiling for cross-attention: the two low scores come from variants of a single internal architecture trained under one tuning budget, and the provider-reported 0.9109 cannot serve as independent corroboration because the released provider model is a multimodal LSTM + CNN with no attention tensors. Whether cross-attention has a ceiling on this dataset is an open question that would need independent implementations at a matched tuning budget.
...
### 7.3. Contribution of the Combined Components

The 2^3 factorial ablation (Table 8) revises the reading of our earlier single-seed component table. Multi-scale temporal differences are the dominant component, accounting for roughly 60% of the V2 → V2+ gain on their own. The supervised contrastive term adds a smaller but consistent gain (+0.42 ± 0.06 pp) that is larger when multi-scale features are present (+0.48 / +0.72 / +0.27 pp across seeds) than on the single-lag baseline (+0.44 / −0.05 / +0.16 pp). SE attention has no detectable main effect; adding it to the multi-scale + contrastive configuration changed F1 by +0.15 ± 0.24 pp over six seeds (positive in five, −0.27 pp in one), which is within the run-to-run spread and does not establish a contribution. Interactions account for about a quarter of the total gain and are of the same order as the seed-to-seed spread, so we do not claim a synergy or a cascading mechanism. The practical conclusion is that the multi-scale difference features should be adopted first, the contrastive term second, and SE attention, whose parameter cost is negligible (+4,240), on the strength of its consistency rather than of a demonstrated gain. Reproducibility note: with deterministic kernels enabled (cuDNN deterministic mode, deterministic algorithms, fixed cuBLAS workspace), two independent runs of the full model with the same seed on different GPUs produced identical per-epoch metrics, identical final weights and F1 = 0.9549; without them a same-seed re-run differs from the first epoch onward (final F1 within 0.1 pp).
...
**Field deployment and higher-resolution thermal input.** Real-time inference performance is estimated from server GPU benchmarks; on-device latency and accuracy on the actual Jetson Orin Nano deployment are pending and will be added in a subsequent revision. As noted in Section 6.5, the field-deployment thermal camera (FLIR Lepton 3.5, 160 x 120 native) exceeds the acquisition camera's native resolution by roughly a factor of 19; fine-tuning on higher-native-resolution thermal images may lift the modest +0.37 % thermal contribution we currently observe, but only if the pixel budget is the binding constraint; as noted in Section 6.5 the current fusion collapses the thermal time axis and combines the two modalities additively, so a resolution increase alone is not sufficient to predict a gain.
...
We presented a lightweight multimodal approach for manufacturing equipment degradation prediction that prioritizes domain-informed feature engineering over architectural complexity. Our multi-scale temporal difference features, combined with supervised contrastive learning and channel attention, achieve a macro F1-score of 0.9557 +/- 0.0006 (mean over three runs) with only 2.85M parameters—nearly 4x fewer than Transformer-based alternatives. The approach achieves 100% detection of safety-critical Severe degradation states and reduces Normal-Mild boundary misclassifications by 27–47% relative to the single-lag variant across three runs.

Our comprehensive ablation study reveals two important insights for practitioners. First, domain-specific feature engineering (temporal differences) provides the single largest performance improvement (+1.95%), outweighing any architectural change. Second, a 2^3 factorial ablation with three seeds and a matched training budget shows that the +1.69 pp F1 gain of the three proposed improvements over the single-lag variant is mostly additive: multi-scale differences contribute about 60% on their own, the contrastive term adds a smaller consistent gain, channel attention has no detectable main effect, and interactions account for about a quarter of the total.

With a measured inference latency of 16.27 ms per sample on an NVIDIA Jetson Orin Nano (ONNX Runtime CUDA execution provider) and a model size of 10.9 MB, the approach is practical for the 1 Hz monitoring cadence targeted here. These findings suggest that in data-limited industrial settings, investing in domain-specific feature engineering yields greater returns than scaling model complexity.
...
[6] *Withdrawn.* This slot previously cited mmTransformer (CVPR 2021), a vehicle trajectory-prediction model that does not address multimodal sensor fusion for predictive maintenance. The number is retained so that references [7] onward keep their identifiers.
